...
class Product
{
    ...
    public function getFilteredProducts($keyword = null, $filters = [], $sort = null, $categoryId = null)
    {
        $sql = "SELECT p.id, p.name, p.price, p.image, p.description, p.brand_id, p.sub_id,
                       COALESCE(STRING_AGG(DISTINCT c.color_code, ','), '') AS color_codes,
                       COALESCE(STRING_AGG(DISTINCT s.size, ','), '') AS sizes,
                       b.name AS brand_name,
                       sc.name AS subcategory_name
                FROM product p
                LEFT JOIN product_size_color psc ON p.id = psc.product_id
        ...
        // 关键修复：明确列出 GROUP BY 字段（PostgreSQL 严格要求）
        $sql .= " GROUP BY p.id, p.name, p.price, p.image, p.description, p.brand_id, p.sub_id, b.name, sc.name";
