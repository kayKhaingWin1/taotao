...
if (isset($_POST['submit'])) {
    ...
    if (empty($error_name) && empty($error_code)) {
        $status = $controller->addItem($type, $name, $color_code);
        if ($status) {
        ...
        <div class="col-md-8 col-lg-6">
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-body p-5">
                    <h3 class="card-title text-center mb-4 text-dark">Add New <?= $title ?></h3>
                    <form method="post">
                        ...
                        <!-- Color Code input only if type=color -->
                        <?php if ($type === 'color'): ?>
                            <div class="mb-3">
                                <label class="form-label">Color Code</label>
                                <input type="text" name="color_code" class="form-control rounded-3" placeholder="#000000" value="<?= htmlspecialchars($color_code) ?>">
                                <?php if ($error_code): ?>
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            <button type="submit" name="submit" class="btn btn-success px-4">Add <?= $title ?></button>
